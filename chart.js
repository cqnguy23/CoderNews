...
fetchCoinData();
let colors = [["blue", "darkblue"],["green", "darkgreen"], ["red", "darkred"]];
function changeColor1(){
    myChart.destroy();
    colors = [["yellow", "orange"],["brown", "darkbrown"], ["grey", "darkgrey"]];
    renderLineGraph(data);
}

function changeColor2(){
    myChart.destroy();
    colors = [["green", "darkgreen"],["red", "darkred"],["blue", "darkblue"]];
    renderLineGraph(data);
}
function changeColor3(){
    myChart.destroy();
    colors = [["red", "darkred"],["green", "darkgreen"],["orange", "darkorange"]];
    renderLineGraph(data);
}
let myChart;
let data;
...
function getPriceChange(data, num) {
    ...
    let bitcoinPricechange1h = calculatePriceFromPercentageChange(bitcoinChanges.price, bitcoinChanges.percent_change_1h)
    let bitcoinPricechange24h = calculatePriceFromPercentageChange(bitcoinChanges.price, bitcoinChanges.percent_change_24h)
    let bitcoinPricechange7d = calculatePriceFromPercentageChange(bitcoinChanges.price, bitcoinChanges.percent_change_7d)
    let bitcoinPricechange30d = calculatePriceFromPercentageChange(bitcoinChanges.price, bitcoinChanges.percent_change_30d)
    let bitcoinPricechange60d = calculatePriceFromPercentageChange(bitcoinChanges.price, bitcoinChanges.percent_change_60d)
    let bitcoinPricechange90d = calculatePriceFromPercentageChange(bitcoinChanges.price, bitcoinChanges.percent_change_90d)
